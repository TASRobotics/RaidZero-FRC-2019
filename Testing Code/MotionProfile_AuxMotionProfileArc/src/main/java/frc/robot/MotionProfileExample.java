...
public class MotionProfileExample {
	 ...
	 */
	public MotionProfileExample(TalonSRX motorController) {
		...
		if (false == IsMotionProfile(_motorController.getControlMode())) {
			...
			switch (_state) {
				...
				case 2: /* check the status of the MP */
					...
					// if (_motorController.getSelectedSensorPosition(0) >= Constants.kSensorUnitsPerRotation * 0.5*MotionProfile.Points[MotionProfile.kNumPoints-1][0] && switcher == false) {
					// 	int start = (int) (0.6*MotionProfile.kNumPoints2);
					// 	int newNumPoints = 0;
					// 	double newPoints[][] = new double[MotionProfile.kNumPoints2-start][4];
						
					// 	for (int i = start; i < MotionProfile.kNumPoints2; i++) {
					// 		newPoints[newNumPoints] = MotionProfile.Points2[i];
					// 		newNumPoints++;
					// 	}
					// 	System.out.println("num points" + newNumPoints);
					// 	switcher = true;
					// 	startFilling(newPoints, newNumPoints);
	...
	private void startFilling(double[][] profile, int totalCnt) {
		...
		PathPoint path[] = PathGenerator.generatePath(points, 10, 20);
		...
		totalCnt = path.length;
		/* This is fast since it's just into our TOP buffer */
		for (int i = 0; i < totalCnt; ++i) {
			double direction = _bForward ? +1 : -1;
			double positionRot = path[i].position;
			double velocityRPM = path[i].velocity;
			double heading = path[i].angle;//_endHeading * positionRot / finalPositionRot; /* scale heading progress to position progress */

			/* for each point, fill our structure and pass it to API */
			point.position = direction * positionRot * Constants.kSensorUnitsPerRotation; 		//Convert Revolutions to Units
			point.velocity = direction * velocityRPM * Constants.kSensorUnitsPerRotation; 	//Convert RPM to Units/100ms
			...
			point.timeDur = 3;//(int) (path[i].time * 10);		// Trajectory Duration is no longer a thing...
			point.zeroPos = false;
			if (i == 0)
